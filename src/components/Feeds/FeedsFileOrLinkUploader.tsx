...
interface FeedsFileOrLinkUploaderProps {
    updatedFeed: string | null;
    onClose: () => void;
    fetchFeeds: () => void;
}


const FeedsFileOrLinkUploader: React.FC<FeedsFileOrLinkUploaderProps> = ({ updatedFeed, onClose, fetchFeeds }) => {
    ...
    const uploadFile = async (file: File) => {
        ...
        formData.append("feedsFile", file);
        setLoading(true);
        setStatus(null);
        ...
        try {
            let res;
            if (updatedFeed) {
                res = await clientSideAxios.put(`/feeds/${updatedFeed}`, formData);
            } else {
                res = await clientSideAxios.post("/feeds", formData);
            }
            const data = res.data;
    ...
    const uploadLink = async () => {
        ...
        try {
            ...
            let res;
            if (updatedFeed) {
                res = await clientSideAxios.put(`/feeds/${updatedFeed}`, { externalUrl: link.trim() });
            } else {
                res = await clientSideAxios.post("/feeds", { externalUrl: link.trim() });
            }
            const data = res.data;
    ...
    return (
        <div className=" w-full mx-auto p-6 rounded-2xl">
            <h2 className="text-xl font-semibold text-center mb-4">{updatedFeed ? "Update" : "Create new"} feed with a File or Paste a Link </h2>
...
export default FeedsFileOrLinkUploader;
