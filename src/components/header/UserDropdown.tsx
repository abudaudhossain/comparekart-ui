...
export default function UserDropdown({ isHiddenName = false, text = "#ffffff" }) {
  ...
  const signoutHandler = async () => {
    try {
      const response = await clientSideAxios.get("/auth/logout")
      setUser(null)
      cookies.remove("jwt");
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className={`sm:block mr-1 font-medium text-theme-sm ${isHiddenName ? "hidden" : "block"}`}>{user?.name}</span>

        <svg
          className={`stroke-gray-500 dark:stroke-gray-400 transition-transform duration-200 ${isOpen ? "rotate-180" : ""
            }  ${isHiddenName ? "hidden" : "sm:block"} sm:block`}
          width="18"
          height="20"
          viewBox="0 0 18 20"
          fill="none"
